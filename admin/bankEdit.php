...
if($num==0){echo"<script>window.location='bank.php';</script>";}
?>
<div style="width:962px; margin:20px auto 0 auto;text-align:right;">
  <input type="button" value="กลับ" onclick="window.location='bank.php'"/>
</div>
<table class="data" style="margin-top:10px;" align="center" cellpadding="10" cellspacing="0" >
  <tr>
    <th class="header">แก้ไขช่องทางการชำระเงิน
<div style="float:right;">
  <a href="process/bank/bankDelete.php?id=<?=base64_encode($row['id'])?>" onclick="return confirm('ลบ ?')"><img src="image/icon-bin.png"class="action" title="ลบ"/></a>
</div>
    </th>
  </tr>
  <tr>
    <td colspan="2">
<form action="process/bank/bankUpdate.php" method="post" name="bank">
<table width="100%" border="0" cellspacing="0" cellpadding="5">
  ...
  <tr>
    <td>เลขที่บัญชี</td>
    <td><input type="text" name="account_number" required value="<?=$row['account_number']?>"/></td>
  </tr>
  <tr>
    <td>ชื่อบัญชี</td>
    <td><input type="text" name="account_name" required value="<?=$row['account_name']?>"/></td>
  </tr>
  <tr>
    <td>สาขา</td>
    <td><input type="text" name="branch" required value="<?=$row['branch']?>"/></td>
  </tr>
  <tr>
    <td>วันที่สมัคร</td>
    <td><input type="text" name="register_date" readonly="readonly" value="<?=substr($row['insert_date'],0,16)?>" style="background:#fafafa;"/></td>
  </tr>
  <tr>
    <td>แก้ไขล่าสุด</td>
    <td><input type="text" name="last_update" readonly="readonly" value="<?=substr($row['last_update'],0,16)?>" style="background:#fafafa;"/></td>
  </tr>
  <tr>
    <td><input name="id" type="hidden" id="id" value="<?=base64_encode($id)?>" /></td>
