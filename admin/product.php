...
//หาจำนวนหน้า
$sql="SELECT COUNT(*) FROM product WHERE name LIKE '%$keyword%' AND status LIKE '%$status%'";
...
<form action="product.php" method="post" name="search">
  ...
  <input type="button" value="ปกติ" onclick="window.location='?keyword=<?=$keyword?>&status=<?=base64_encode('ป')?>'"/>
  <input type="button" value="ขายดี" onclick="window.location='?keyword=<?=$keyword?>&status=<?=base64_encode('ข')?>'"/>
  <input type="button" value="หมด" onclick="window.location='?keyword=<?=$keyword?>&status=<?=base64_encode('มด')?>'"/>  
