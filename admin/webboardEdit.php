<?
if(empty($_REQUEST['id'])){echo"<script>window.location='webboard.php';</script>";exit;}
session_start();
?>
...
$sql="SELECT * FROM webboard_post WHERE id=$id";
$result=mysql_query($sql)or die(mysql_error());
$row=mysql_fetch_array($result);
?>
<table class="data" style="margin-top:10px;" align="center" cellpadding="10" cellspacing="0">
  <tr>
    <th class="header" colspan="2">หัวข้อ</th>
  </tr>
  <tr>
    <td colspan="2">
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="webboard">
  <tr>
  ...
  <tr>
    ...
    <td>
      ...
      <a href="webboardPostEdit.php?id=<?=base64_encode($row['id'])?>"><img src="image/icon-pencil.png" class="action" title="แก้ไข"/></a>
...
?>
  <tr>
    <td align="center" style="border-top:solid 1px #dfdfdf;">
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="webboard">
  <tr>
...
?>
  <tr>
    <td style="border-top:solid 1px #dfdfdf;">
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="webboard">
  <tr>
    <td width="100">รายละเอียด</td>
    <td><?=$rowReply['detail']?></td>
  </tr>
  <tr>
    <td>ผู้โพส</td>
    <td><?=$rowReply['username']?></td>
  </tr>
  <tr>
    <td>วันที่โพส</td>
    <td><?=$rowReply['insert_date']?></td>
  </tr>
  <tr>
    <td>แก้ไขล่าสุด</td>
    <td><?=$rowReply['last_update']?></td>
  </tr>
  <tr>
    <td>จัดการ</td>
    <td><a href="webboardReplyEdit.php?id=<?=base64_encode($rowReply['id'])?>"><img src="image/icon-pencil.png" class="action" title="แก้ไข"/></a>
      <a href="process/webboard/replyDelete.php?id=<?=base64_encode($rowReply['id'])?>" onclick="return confirm('ลบ ?')"><img src="image/icon-bin.png"class="action" title="ลบ"/></a>
    </td>
  </tr>
</table>
    </td>
...
<table class="data" align="center" cellpadding="10" cellspacing="0">
  <tr>
    <th class="header" colspan="2">ตอบหัวข้อนี้</th>
  </tr>
  <tr>
    <td colspan="2">
<form action="process/webboard/replyInsert.php" method="post" name="webboard" >
<table width="100%" border="0" align="center" cellpadding="5" cellspacing="0">
  ...
  <tr>
    <td><input name="id" type="hidden" value="<?=$_REQUEST['id']?>" /></td>
