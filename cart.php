...
<title>ตะกร้าสินค้า | <?php $rowSetting['title']?></title>
...
?>
  <tr>
    <td align="center">
      <input class="form" type="checkbox" name="remove[]" value="<?php=$id?>" />
    </td>
    <td align="center">
      <a href="สินค้า-<?php=$item['id']."-".rewrite_url($item['name'])?>.html" title="<?php=$item['name']?>" target="_blank"><img src="product/<?php=$item['image']?>" width="90" border="0"/></a>
    </td>
    <td align="center"><?php=$item['name']?><br/><?php=$item['option']?></td>
    <td align="center"><?php=number_format($item['price'])?></td>
    <td align="center">
      <a href="javascript:;"onclick="del('<?php=$i+1?>')" style="display:block; margin-bottom:5px;"><img src="image/icon-minus.gif" border="0"/></a>
      <input class="form" style="width:20px; text-align:center;" name="amount[<?php=$id?>]" type="text" id="<?php=$i+1?>" value="<?php=$item['amount']?>"/>
      <a href="javascript:;" onclick="plus('<?php=$i+1?>')" style="display:block;margin-top:5px;"><img src="<?php=$rowSetting['website_name']."/"?>image/icon-plus.gif" border="0" /></a>
    </td>
    <td align="right"><?php=number_format($item['price']*$item['amount'])?></td>
  </tr>
<?php
    $sum+=$item['price']*$item['amount'];
    $quantity+=$item['amount'];
...
?>
  <tr>
    ...
    <td style="font-size:16px;background:#fafafa;" align="right"><?php=number_format($sum)?></td>
  </tr>
</table>
<div align="center" style="padding:15px 0 0 0;">
  <input type="button" onclick="history.back();" value="เลือกสินค้าต่อ" />
...
?>
<div align="center">ไม่มีสินค้าในตะกร้า<br/><input name="button" type="button"  onclick="history.back();" value="เลือกสินค้าต่อ" style="margin:10px 0 0 0;"/></div>
<?php
}
?>
    </td>
