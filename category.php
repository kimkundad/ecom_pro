...
$numShowCategory=mysql_num_rows($resultShowCategory);
if($numShowCategory==0){$rowShowCategory['name']='ไม่มีหมวดหมู่';}
?>
...
<table class="box" width="100%" border="0" cellspacing="0" cellpadding="10">
  <tr>
    <th style="text-align:left;">รายการสินค้าหมวด <?=$rowShowCategory['name']?></th>
  </tr>
  <tr>
    <td>
...
if($page>0){
?>
  <input type="button" value="ก่อนหน้า" onclick="window.location='หมวดหมู่-<?=$category_id."-".rewrite_url($rowShowCategory['name'])."-".$page?>.html'"/>
<? 
}  
...
?>
    <a href="หมวดหมู่-<?=$category_id."-".rewrite_url($rowShowCategory['name'])."-".($i+1)?>.html" class="number"><?=$i+1?></a>
<?   
  }
}
if($page<$num_page-1){
?>
    <input type="button" value="ถัดไป" onclick="window.location='หมวดหมู่-<?=$category_id."-".rewrite_url($rowShowCategory['name'])."-".($page+2)?>.html'"/>
<? 
} 
?>
